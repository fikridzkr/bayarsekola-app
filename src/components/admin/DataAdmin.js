...
const DataAdmin = () => {
  ...
  const deleteDataAdmin = (id) => {
    Axios.delete(`http://localhost:3001/admin/delete/${id}`).then((res) => {
      setDataAdmin(
        dataAdmin.filter((value) => {
          return value.id != id;
        })
      );
    });
